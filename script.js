...
function handleTacticsInteraction(e) {
    ...
    // Se clicou em um jogador
    if (playerId) {
        const player = gameState.userClub.players.find(p => p.name === playerId);
        const sourceInfo = getPlayerLocation(player);

        if (selectedPlayerInfo) { // Se já havia um jogador selecionado...
            if (selectedPlayerInfo.player.name === player.name) { // Clicou no mesmo jogador
                clearSelection();
            } else { // Clicou em outro jogador para trocar
                swapPlayers(selectedPlayerInfo, sourceInfo);
                clearSelection();
            }
        } else { // Se não havia ninguém selecionado...
            selectPlayer(player, sourceInfo.type, sourceInfo.id);
        }
    } else if (selectedPlayerInfo) { // Se clicou em um local vazio (slot/banco/reservas) com um jogador selecionado...
        let destInfo;
...
function swapPlayers(sourcePlayerInfo, destPlayerInfo) { removePlayerFromSource(sourcePlayerInfo); removePlayerFromSource(destPlayerInfo); addPlayerToDest(sourcePlayerInfo.player, {type: destPlayerInfo.type, id: destPlayerInfo.id}); addPlayerToDest(destPlayerInfo.player, {type: sourcePlayerInfo.sourceType, id: sourcePlayerInfo.sourceId}); loadTacticsScreen(); }

function calculateModifiedOverall(player, targetPosition) { if (!player || !targetPosition || !player.position) return player.overall; const naturalPosition = player.position; if (!positionMatrix[naturalPosition]) return player.overall; const penaltyFactor = 4; const cleanTargetPosition = targetPosition.replace(/\d/g, '').replace('RWB', 'RB').replace('LWB', 'LB').replace('RM', 'RW').replace('LM', 'LW'); if (!positionMatrix[naturalPosition][cleanTargetPosition]) return player.overall; const distance = positionMatrix[naturalPosition][cleanTargetPosition]; const penalty = distance * penaltyFactor; return Math.max(40, player.overall - penalty); }
...
function updateCalendar() { const container = document.getElementById('calendar-container'); const date = gameState.currentDate; const month = date.getMonth(); const year = date.getFullYear(); const firstDay = new Date(year, month, 1); const lastDay = new Date(year, month + 1, 0); let html = `<div class="calendar-header"> <button id="prev-month-btn">◄</button> <h3>${date.toLocaleString('pt-BR', { month: 'long', year: 'numeric' }).toUpperCase()}</h3> <button id="next-month-btn">►</button> </div> <div class="calendar-grid"><div class="calendar-weekday">Dom</div><div class="calendar-weekday">Seg</div><div class="calendar-weekday">Ter</div><div class="calendar-weekday">Qua</div><div class="calendar-weekday">Qui</div><div class="calendar-weekday">Sex</div><div class="calendar-weekday">Sáb</div>`; for (let i = 0; i < firstDay.getDay(); i++) { html += `<div class="calendar-day other-month"></div>`; } for (let i = 1; i <= lastDay.getDate(); i++) { const loopDate = new Date(year, month, i); const isCurrent = isSameDay(loopDate, gameState.currentDate); const matchOnThisDay = gameState.schedule.find(m => isSameDay(new Date(m.date), loopDate) && (m.home.name === gameState.userClub.name || m.away.name === gameState.userClub.name)); let dayClasses = 'calendar-day'; let dayContent = `<span class="day-number">${i}</span>`; if (matchOnThisDay) { dayClasses += ' match-day'; const opponent = matchOnThisDay.home.name === gameState.userClub.name ? `(C) ${matchOnThisDay.away.name}` : `(F) ${matchOnThisDay.home.name}`; dayContent += `<div class="match-details">${opponent}</div>`; } if (isCurrent) { dayClasses += ' current-day'; } html += `<div class="${dayClasses}">${dayContent}</div>`; } html += '</div>'; container.innerHTML = html; document.getElementById('prev-month-btn').addEventListener('click', () => { gameState.currentDate.setMonth(gameState.currentDate.getMonth() - 1); updateCalendar(); }); document.getElementById('next-month-btn').addEventListener('click', () => { gameState.currentDate.setMonth(gameState.currentDate.getMonth() + 1); updateCalendar(); }); }
...
function updateContinueButton() { const button = document.getElementById('advance-day-button'); const displayDate = document.getElementById('current-date-display'); displayDate.innerText = gameState.currentDate.toLocaleDateString('pt-BR', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' }); if (gameState.nextUserMatch && isSameDay(gameState.currentDate, new Date(gameState.nextUserMatch.date))) { button.innerText = "JOGAR PARTIDA"; button.disabled = true; // Desabilitar temporariamente até a lógica da partida estar pronta } else { button.innerText = "Avançar"; button.disabled = false; } }
function simulateDayMatches() { const todayMatches = gameState.schedule.filter(match => isSameDay(new Date(match.date), gameState.currentDate)); for (const match of todayMatches) { if (match.status === 'scheduled') { if (match.home.name !== gameState.userClub.name && match.away.name !== gameState.userClub.name) { match.homeScore = Math.floor(Math.random() * 4); match.awayScore = Math.floor(Math.random() * 4); match.status = 'played'; updateTableWithResult(match); } } } findNextUserMatch(); }
...
function generateSchedule(teams, leagueInfo) { let clubes = [...teams]; if (clubes.length % 2 !== 0) { clubes.push({ name: "BYE", logo: '', players: [] }); } const numTeams = clubes.length; const numRounds = (numTeams - 1) * 2; const matchesPerRound = numTeams / 2; const allMatches = []; let roundRobinTeams = [...clubes]; for (let round = 0; round < numTeams - 1; round++) { for (let match = 0; match < matchesPerRound; match++) { const home = roundRobinTeams[match]; const away = roundRobinTeams[numTeams - 1 - match]; if (home.name !== "BYE" && away.name !== "BYE") { allMatches.push({ home, away }); } } roundRobinTeams.splice(1, 0, roundRobinTeams.pop()); } const secondHalfMatches = allMatches.map(match => ({ home: match.away, away: match.home })); const fullScheduleRaw = [...allMatches, ...secondHalfMatches]; const schedule = []; let currentDate = new Date(leagueInfo.startDate + 'T12:00:00Z'); for (let i = 0; i < fullScheduleRaw.length; i++) { if (i > 0 && i % matchesPerRound === 0) { const currentDay = currentDate.getDay(); if (currentDay === 6) { currentDate.setDate(currentDate.getDate() + 4); } else if (currentDay === 3) { currentDate.setDate(currentDate.getDate() + 3); } else { currentDate.setDate(currentDate.getDate() + 7); } } schedule.push({ ...fullScheduleRaw[i], date: new Date(currentDate).toISOString(), status: 'scheduled' }); } return schedule.sort((a, b) => new Date(a.date) - new Date(b.date)); }
