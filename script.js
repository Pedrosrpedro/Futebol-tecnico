...
const gameState = {
    ...
    tactics: {
        formation: '4-2-3-1',
        mentality: 'balanced',
        // Novas opções táticas detalhadas
...
// --- Estado da Interface (NOVO) ---
let selectedPlayerInfo = null; // { player, sourceType, sourceId }
...
function handleTacticsInteraction(e) {
    ...
    if (playerId) {
        ...
        if (selectedPlayerInfo) {
            // Se o jogador clicado é o mesmo que já está selecionado, deseleciona
            ...
            // Nenhum jogador selecionado, então seleciona este
            selectPlayer(player, sourceInfo.type, sourceInfo.id);
        }
    } 
    // CASO 2: Clicou em um lugar vazio (slot ou lista)
...
function selectPlayer(player, sourceType, sourceId) {
    ...
    // Adiciona a classe visual para feedback
...
function removePlayerFromSource(playerInfo) {
    if (playerInfo.sourceType === 'field') {
        ...
        gameState.squadManagement.substitutes = gameState.squadManagement.substitutes.filter(p => p.name !== playerInfo.player.name);
    } else {
        gameState.squadManagement.reserves = gameState.squadManagement.reserves.filter(p => p.name !== playerInfo.player.name);
    }
}
...
function movePlayer(playerInfo, destInfo) {
    // Validação para o banco de reservas
...
function swapPlayers(sourcePlayerInfo, destPlayerInfo) {
    ...
    removePlayerFromSource({player: destPlayerInfo.player, sourceType: destPlayerInfo.type, sourceId: destPlayerInfo.id});
    
    addPlayerToDest(sourcePlayerInfo.player, destPlayerInfo);
...
// --- Funções de Renderização de Táticas ---

function calculateModifiedOverall(player, targetPosition) {
    if (!player || !targetPosition) return player.overall;
    const naturalPosition = player.position;
    const penaltyFactor = 4; 
    const distance = positionMatrix[naturalPosition]?.[targetPosition.replace(/\d/g, '')] ?? 4;
...
function loadTacticsScreen() {
    ...
    for (const pos in positions) {
        ...
        slot.style.top = `${positions[pos][0] - 6}%`;
        slot.style.left = `${positions[pos][1] - 5}%`;
    ...
    // Popula listas de banco e reservas
    ...
    // Se um jogador estava selecionado, mantém a classe visual
    if (selectedPlayerInfo) {
        const element = document.querySelector(`[data-player-id="${selectedPlayerInfo.player.name}"]`);
        if (element) element.classList.add('selected');
    }
...
function createPlayerChip(player, currentPosition) {
    ...
    if (modifiedOverall < player.overall) overallClass += ' penalty';
...
function initializeEventListeners() {
    ...
    document.querySelectorAll('#tactics-content select').forEach(select => {
        select.addEventListener('change', (e) => {
            ...
            // Lógica especial para mudança de formação
