...
function swapPlayers(sourcePlayerInfo, destPlayerInfo) { removePlayerFromSource(sourcePlayerInfo); removePlayerFromSource({player: destPlayerInfo.player, sourceType: destPlayerInfo.type, sourceId: destPlayerInfo.id}); addPlayerToDest(sourcePlayerInfo.player, destPlayerInfo); addPlayerToDest(destPlayerInfo.player, {type: sourcePlayerInfo.sourceType, id: sourcePlayerInfo.sourceId}); loadTacticsScreen(); }

function calculateModifiedOverall(player, targetPosition) { if (!player || !targetPosition) return player.overall; const naturalPosition = player.position; if (!positionMatrix[naturalPosition]) return player.overall; const penaltyFactor = 4; const cleanTargetPosition = targetPosition.replace(/\d/g, ''); const distance = positionMatrix[naturalPosition][cleanTargetPosition] ?? 4; const penalty = distance * penaltyFactor; return Math.max(40, player.overall - penalty); }
